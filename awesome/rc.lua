...
run_once({ "unclutter -root", "nm-applet", "blueman-applet"}) -- entries must be separated by commas
...
-- bavery_like: 4, 5, 6 (but no binary clock), 7, love 10
...
function set_maximized(c)
        if c.maximized then
                --naughty.notify({text=string.format("Maximized %s", c.name), timeout=30})
                local t = awful.tag.add("M:" .. c.class, {screen= c.screen, volatile=true})
                c:tags({t})
                t:view_only()
        else
                --naughty.notify({text=string.format("Unmaximized %s", c.name), timeout=30})
...
client.connect_signal("property::maximized", set_maximized)
